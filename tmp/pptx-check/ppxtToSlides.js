...
const annotateContentsTexts = (slide, metas) => {
    ...
    for (const meta of remaining) {
        ...
        if (!meta.element.groupId)
            setGroupId(meta.element, nanoid(10));
        groups.push([meta]);
    }
    return groups;
};
 ...
 */
export function convertPPTXToSlides(json, options = {}) {
 ...
 * 检测幻灯片类型
 */
export function detectSlideType(slide, index, totalSlides) {
    if (index === 0)
        return 'cover';
 ...
 * 为slides添加类型标记
 */
export function enrichSlidesWithType(slides) {
...
//   }
// }
export function toPPTistTemplate(data) {
    const slides = enrichSlidesWithType(data.slides);
    return {
        title: '未命名演示文稿',
        width: 1000,
