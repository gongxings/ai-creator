...
exports.convertPPTXToSlides = convertPPTXToSlides;
exports.detectSlideType = detectSlideType;
exports.enrichSlidesWithType = enrichSlidesWithType;
exports.toPPTistTemplate = toPPTistTemplate;
/**
 * PPTX转换工具 - 将pptxtojson结果转换为PPTist slides格式
 * 核心转换逻辑参考自PPTist的useImport.ts
 */
const nanoid_1 = require("nanoid");
const shapeVAlignMap = {
    'mid': 'middle',
    'down': 'bottom',
    'up': 'top',
};
const fontFamilyAliasMap = {
...
const getParagraphMetrics = (html, ratio) => {
    ...
    while ((match = tagRegex.exec(html)) !== null) {
        ...
        if (tagEndIndex === -1)
            tagEndIndex = tagStartIndex + fullTag.length;
...
const getMaxFontSize = (html, defaultFontSize = 18) => {
    const fontSizeRegex = /font-size\s*:\s*(\d+(?:\.\d+)?)\s*pt/gi;
...
const sanitizeSvgPath = (path) => {
    return path
        .replace(/NaN/g, '0')
        .replace(/z{2,}/gi, ' z')
        .trim();
};
const hasMeaningfulText = (content) => {
    if (!content)
        return false;
...
const canConvertPatternShapeToImage = (el) => {
    if (el.type !== 'shape' || el.fill?.type !== 'image' || !el.fill.value?.picBase64)
        return false;
    if (hasMeaningfulText(el.content))
        return false;
...
const flipGroupElements = (elements, axis) => {
    ...
    return elements.map(element => {
        const next = { ...element };
        if (axis === 'y')
            next.left = 2 * centerX - element.left - element.width;
        if (axis === 'x')
            next.top = 2 * centerY - element.top - element.height;
        return next;
    });
};
const getGroupBounds = (elements) => {
...
const calculateRotatedPosition = (ax, ay, aw, ah, bx, by, bw, bh, ak, bk) => {
...
const calculateRotatedPositionByBounds = (ax, ay, minLocalX, minLocalY, aw, ah, bx, by, bw, bh, ak, bk) => {
    const aRadians = ak * (Math.PI / 180);
    const aCos = Math.cos(aRadians);
    const aSin = Math.sin(aRadians);
...
const getElementTextContent = (el) => {
    ...
    if (el.type === 'shape' && el.text?.content)
        return stripHtml(el.text.content);
    return '';
};
const getElementTextHtml = (el) => {
    if (el.type === 'text')
        return el.content || '';
    if (el.type === 'shape' && el.text?.content)
...
const getElementFontSize = (el) => {
    const html = getElementTextHtml(el);
    const match = html.match(/font-size:\s*([\d.]+)px/i);
    return match ? parseFloat(match[1]) : 16;
};
const setTextType = (el, type) => {
    if (el.type === 'text')
        el.textType = type;
    if (el.type === 'shape' && el.text)
        el.text.type = type;
};
const setGroupId = (el, groupId) => {
    el.groupId = groupId;
};
const buildTextMetas = (slide) => {
...
const findNearestMeta = (target, candidates) => {
...
const annotateContentTexts = (slide, metas) => {
    if (!metas.length)
        return [];
    const title = metas
    ...
    if (title)
        setTextType(title.element, 'title');
    ...
    while (index < textCandidates.length) {
        ...
        if (looksLikePair) {
            const groupId = (0, nanoid_1.nanoid)(10);
    ...
    for (const numberMeta of numbered) {
        ...
        const groupId = nearest?.element.groupId || (0, nanoid_1.nanoid)(10);
        setGroupId(numberMeta.element, groupId);
        if (nearest && !nearest.element.groupId)
            setGroupId(nearest.element, groupId);
    }
    return groups;
};
const annotateContentsTexts = (slide, metas) => {
    if (!metas.length)
        return [];
    const title = metas
        .sort((a, b) => b.fontSize - a.fontSize || a.centerY - b.centerY)[0];
    ...
    for (const meta of remaining) {
        ...
        if (!meta.element.groupId)
            setGroupId(meta.element, (0, nanoid_1.nanoid)(10));
        groups.push([meta]);
    }
    return groups;
};
const annotateCoverLikeTexts = (slide, metas, slideType) => {
    if (!metas.length)
        return [];
    ...
    if (slideType === 'transition') {
        ...
        if (partNumber)
            setTextType(partNumber.element, 'partNumber');
    }
    const content = sorted.find(item => item.element.id !== title?.element.id &&
        item.element.textType !== 'partNumber' &&
        item.centerY >= (title?.centerY || 0));
    if (content)
        setTextType(content.element, 'content');
    return [];
};
const annotateTemplateSemantics = (slides) => {
    return slides.map((slide, index) => {
        ...
        let groups = [];
        if (type === 'content')
            groups = annotateContentTexts(slide, metas);
        else if (type === 'contents')
            groups = annotateContentsTexts(slide, metas);
        else
            groups = annotateCoverLikeTexts(slide, metas, type);
 ...
 */
function convertPPTXToSlides(json, options = {}) {
    ...
    for (const item of json.slides || []) {
        ...
        if (slideFill?.type === 'image' && slideFill.value?.picBase64) {
            ...
            background = {
                ...
                gradient: {
                    ...
                    colors: (slideFill.value.colors || []).map((c) => ({
                        ...c,
                        pos: parseInt(c.pos),
                    })),
        ...
        const isCorruptedFullscreenShape = (el) => {
        ...
        const parseElements = (elements) => {
            ...
            for (const el of sortedElements) {
                ...
                if (el.type === 'group') {
                    ...
                    let groupElements = (el.elements || []).map((_el) => {
                        let left = (_el.left || 0) + originLeft;
                        let top = (_el.top || 0) + originTop;
                        let rotate = _el.rotate || 0;
                        if (el.rotate) {
                            const position = calculateRotatedPositionByBounds(originLeft, originTop, childBounds.minX, childBounds.minY, childBounds.width || originWidth, childBounds.height || originHeight, _el.left || 0, _el.top || 0, _el.width || 0, _el.height || 0, el.rotate, rotate);
                            left = position.x;
                            top = position.y;
                        ...
                        if (el.isFlipH && 'isFlipH' in element)
                            element.isFlipH = true;
                        if (el.isFlipV && 'isFlipV' in element)
                            element.isFlipV = true;
                        if ('rotate' in element && el.rotate)
                            element.rotate = rotate;
                        return element;
                    });
                    if (el.isFlipH)
                        groupElements = flipGroupElements(groupElements, 'y');
                    if (el.isFlipV)
                        groupElements = flipGroupElements(groupElements, 'x');
                    parseElements(groupElements);
                    continue;
                }
                if (el.type === 'diagram') {
                    const diagramElements = (el.elements || []).map((_el) => ({
                        ..._el,
                ...
                if (isCorruptedFullscreenShape(el))
                    continue;
                const convertedEl = {
                    id: (0, nanoid_1.nanoid)(10),
                ...
                if (el.type === 'text') {
                    ...
                    if (metrics.lineHeight)
                        convertedEl.lineHeight = metrics.lineHeight;
                    if (metrics.margin)
                        convertedEl.paragraphSpace = metrics.margin;
                }
                    ...
                    if (canConvertPatternShapeToImage(el)) {
                        ...
                        if (el.isFlipH)
                            convertedEl.flipH = true;
                        if (el.isFlipV)
                            convertedEl.flipV = true;
                    }
                    else if (el.shapType === 'line' || /Connector/.test(el.shapType)) {
                        convertedEl.type = 'line';
                        let start = [0, 0];
                        let end = [convertedEl.width, convertedEl.height];
                        ...
                        if (el.fill?.type === 'color' && el.fill.value) {
                            ...
                            convertedEl.gradient = {
                                ...
                                colors: el.fill.value.colors.map((c) => ({
                                    ...c,
                                    pos: parseInt(c.pos),
                                })),
                        ...
                        if (el.content) {
                            ...
                            const metrics = getParagraphMetrics(el.content, ratio);
                            if (metrics.lineHeight)
                                convertedEl.text.lineHeight = metrics.lineHeight;
                            if (metrics.margin)
                                convertedEl.text.paragraphSpace = metrics.margin;
                        }
                        if (el.isFlipH)
                            convertedEl.flipH = true;
                        if (el.isFlipV)
                    ...
                    convertedEl.colWidths = (el.colWidths || []).map((w) => w / allWidth);
                    ...
                    if (el.data) {
                        ...
                        const legends = el.data?.map((chartItem) => chartItem.key) || [];
                        const series = el.data?.map((chartItem) => chartItem.values?.map((v) => v.y) || []) || [];
 ...
 * 为slides添加类型标记
 */
function enrichSlidesWithType(slides) {
...
//   }
// }
function toPPTistTemplate(data) {
    const slides = enrichSlidesWithType(data.slides);
    return {
        title: '未命名演示文稿',
        width: 1000,
