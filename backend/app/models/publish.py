...
class PlatformAccount(Base):
    ...
    # 关系（不使用外键）
    user = relationship("User", back_populates="platform_accounts", foreign_keys=[user_id], primaryjoin="PlatformAccount.user_id == User.id")
...
class PublishRecord(Base):
    ...
    # 关系（不使用外键）
    user = relationship("User", back_populates="publish_records", foreign_keys=[user_id], primaryjoin="PublishRecord.user_id == User.id")
    creation = relationship("Creation", back_populates="publish_records", foreign_keys=[creation_id], primaryjoin="PublishRecord.creation_id == Creation.id")
    platform_account = relationship("PlatformAccount", back_populates="publish_records", foreign_keys=[platform_account_id], primaryjoin="PublishRecord.platform_account_id == PlatformAccount.id")
