...
class AIModel(Base):
    ...
    # 关系（不使用外键）
    user = relationship("User", back_populates="ai_models", foreign_keys=[user_id], primaryjoin="AIModel.user_id == User.id")
    creations = relationship("Creation", back_populates="model", foreign_keys="Creation.model_id", primaryjoin="AIModel.id == Creation.model_id")
